...
final class CSSUnitTests: XCTestCase {
    ...
    func testUnitParsing() throws {
        do {
            _ = try CSSParser.parse(attributeName: "unit-tester", attributeValue: "10tt", validationConfiguration: StubCSSValidationConfiguration())
        }
    ...
    func testLengthUnitConversion() throws {
        ...
        do {
            _ = try CSSLengthUnit.ch.convert(amount: 1, to: .cm)
        ...
        do {
            _ = try CSSLengthUnit.cm.convert(amount: 1, to: .ch)
            XCTFail("Test should fail")
        }
        catch CSSLengthUnitErrors.notAbsoluteLength(let unit) {
            XCTAssertEqual(unit, .ch)
